...
const getCreditNotes = async () => {
  const response = await api.get(`/Facturacion/MostrarNotasCredito`, config);
...
const getDataForms = async (UID) => {
  const response = await api.get(
    `/Facturacion/DatosNotaCredito?uid=${UID}`,
    config
  );
  return response.data;
};
...
const insertPaidAddon = async (ID, object) => {
  const resppnse = await api.post(
    `/Facturacion/CrearFactura?ordenVentaID=${ID}`,
    object,
    config
  );
