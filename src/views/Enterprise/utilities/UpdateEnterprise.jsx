...
const UpdateEnterprise = () => {
  ...
  useEffect(() => {
    requests.getEnterprise(enterpriseID).then((response) => {
  ...
  const handleUpdateEnterpriseClick = async (values, { setSubmitting }) => {
    try {
      const object = {
      ...
      const log = {
        ...
        Accion: "Editar",
        Contenido: `Edito la empresa ${values.name}.`,
      };

      const response = await requests.updateEnterprises(object);
  ...
  return (
    <div className="flex flex-col gap-2 m-4">
      ...
      <div className="bg-white w-max flex flex-col gap-2 p-5 rounded-md shadow-md">
        ...
        <Formik
          initialValues={{
            name: enterpriseData?.Nombre || "",
            rfc: enterpriseData?.RFC || "",
          }}
          validationSchema={validationSchema}
          onSubmit={handleUpdateEnterpriseClick}
