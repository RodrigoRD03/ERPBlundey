...
import { TbPdf, TbSearch, TbUpload } from "react-icons/tb";
...
const ViewReferrals = ({ id, close }) => {
  ...
  const [loadingSignedId, setLoadingSignedId] = useState(null);
  ...
  const refetch = async () => {
    const response = await requests.getReferralsSale(id);
    setListPendingSales(response);
  };
  ...
  const handleGetdocumentSignedClick = async (referralID) => {
    try {
      setLoadingSignedId(referralID); 
      const response = await requests.getReferralDocumentSigned(referralID);
      setViewDoc(response);
    } catch (error) {
      console.error("Error al cargar documento:", error);
    } finally {
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      renderCell: (item) => (
        <div className="font-bold capitalize flex justify-center items-center gap-3">
          {item.ExistePDF ? (
            <Tooltip content="Ver PDF firmado">
              <Button
                variant="soft"
                color="cyan"
                onClick={() => handleGetdocumentSignedClick(item.ID)}
                disabled={loadingSignedId === item.ID}
              >
                {loadingSignedId === item.ID ? (
                  // 👇 Loader solo en el botón clickeado
                  <Spinner size="small" />
                ) : (
                  <>
            ...
            <Tooltip content="Subir PDF">
              <Button
                variant="soft"
                color="green"
                onClick={() => setUploadPDF(item.ID)}
                disabled={loadingId === item.ID}
              >
                <TbUpload size={22} />
              </Button>
  ...
  return (
    <>
      ...
      {uploadPDF && <UploadPDF id={uploadPDF} close={() => setUploadPDF(null)} reload={() => refetch()} />}
    </>
  );
};
