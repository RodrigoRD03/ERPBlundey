...
const UpdateRetreat = () => {
  const { retreatID, type } = useParams();
  const [retreatData, setRetreatData] = useState({});
  const navigate = useNavigate();

  useEffect(() => {
    if (type === "Nacional") {
      requests.getRetreatNational(retreatID).then((response) => {
        setRetreatData(response);
      });
    }
    if (type === "Dolar") {
      requests.getRetreatDollar(retreatID).then((response) => {
        setRetreatData(response);
      });
    }
  }, []);
  ...
  const validationSchema = Yup.object().shape({
    document: Yup.string()
      .oneOf(["Transferecia", "Cargo", "Check"], "Seleccione una opción válida")
  ...
  const handleInsertSubscriptionClick = async (values, { setSubmitting }) => {
    try {
      const object = {
        ...
        CuentaBancariaID: retreatData.CuentaBancariaID,
      };

      if (type === "Nacional") {
        const response = await requests.updateRetreatNational(object);

        if (response == true) {
          navigate("/Layout/Retreats");
        }
      } else if (type === "Dolar") {
        const response = await requests.updateRetreatDollar(object);
    ...
    <div className="flex flex-col gap-2 m-4">
      <div className="w-full flex flex-col gap-1">
        <p className="text-xs">
          ...
          <Link to="/Layout/Retreats">Retiros</Link> / <b>Actualizar Retiro</b>
        </p>
        <p className="text-lg font-bold tracking-wide">Actualizar Retiro</p>
      </div>
      ...
      <div className="bg-white w-max flex flex-col gap-2 p-5 rounded-md shadow-md">
        ...
        <Formik
          initialValues={{
            document: retreatData.Documento,
            concept: retreatData.Concepto,
            remarks: retreatData.Observaciones,
            quantity: retreatData.Abono,
            numberCheck: "",
          }}
