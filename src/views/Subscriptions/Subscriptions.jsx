...
const Subscriptions = () => {
  ...
  useEffect(() => {
    requests.getPaymentsNational().then((response) => {
      setListNational(response);
    });
    requests.getPaymentsDollar().then((response) => {
    ...
    },
    {
      ...
      renderCell: (item) => (
        <div className="font-normal flex justify-center items-center">
          ...
          >
            <Button variant="soft" color="lime" onClick={() => navigate(`/Layout/Subscriptions/Update/${item.ID}/${item.Tipo}`)}>
              <TbEdit size={24} />
            </Button>
          </Tooltip>
        </div>
  ...
  const dataNational = {
    nodes: listNational
      ...
      .map((item) => ({ ...item, id: item.ID, Moneda: "MXN", Tipo: "Nacional" })),
  };
  ...
  return (
    <div className="flex flex-col gap-2 m-4">
      <div className="w-full flex flex-col gap-1">
        <p className="text-xs">
          <Link to="/Layout">Panel de Control</Link> / <b>Abonos</b>
        </p>
        <p className="text-lg font-bold tracking-wide">Abonos</p>
      </div>

      <div className="line-row" />

      <Link to="/Layout/Subscriptions/Add" className={`${GlobalStyles.btnAdd}`}>
        <p className="text-sm">Nuevo Abono</p>
        <span>
          <GoPlus size={24} />
        </span>
      </Link>
      ...
      {/* Tabla Nacional */}
      <p className="text-lg font-bold tracking-wide mt-2">Abonos Nacionales</p>
      ...
      {/* Tabla Dólares */}
      <p className="text-lg font-bold tracking-wide ">Abonos en Dólares</p>
